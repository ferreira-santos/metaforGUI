    ...
    # load Dataset file
      ...
      if(length(tag(win, "f"))>0) {
        ...
        if(Sys.localeconv()["decimal_point"]==".") {
          tag(win,"dataset") <<- read.csv(tag(win, "f"))
          assign("dataset", tag(win,"dataset"), envir=.GlobalEnv)
          }
        else {
          tag(win,"dataset") <<- read.csv2(tag(win, "f"))
          assign("dataset", tag(win,"dataset"), envir=.GlobalEnv)
          }
      }
...
gbutton("Run Meta-Analysis", container=g2frame, handler=
  ...
  if(svalue(outputMAobjData)==T) {
    save(tag(win,"meta_analysis"), file="metaforGUI_MetaAnalysis.RData")
